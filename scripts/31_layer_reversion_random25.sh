#!/usr/bin/env bash
# RANDOM CONTROL for the layer-targeted study.
#
# Identical to _layer_reversion_chain.sh except that the 63 layers are drawn at
# random (fixed seed) instead of being the highest-drift quartile. The layer COUNT
# is held constant, so any difference between the two arms is attributable to which
# layers were chosen, not to how many. Same dials, same adapters, same evaluation.
#
...
for J in "${JOBS[@]}"; do
  ...
  SEL=$OUT/selected_layers_${MODE}${SEED}_q${QUANTILE}.json
  ...
  if [ -f "$OUT/layer-${MODE}${SEED}-dirsweep__both__val.json" ]; then
    echo "  already swept -- skipping"; continue
  fi
  $PY $CODE/Identification_and_Reversion/layer_select.py \
      --run $KEY --quantile $QUANTILE --mode $MODE --seed $SEED --stat rho_median --out $SEL

  pool_submit "lr_${MODE}${SEED}_${RUN}" \
...
$PY -u Thesis_Experiment/pipeline/layer_sweep.py \
  ...
  --out $OUT/layer-${MODE}${SEED}-dirsweep__both__val.json \
  --progress $JOBLOG/lr_${MODE}${SEED}_${RUN}.progress\""
done
pool_wait
echo "LAYER REVERSION (RANDOM CONTROL) DONE $(date -Is)" | tee $OUTROOT/DONE_random
