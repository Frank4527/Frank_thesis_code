...
# 4. NEW BASELINE — joint: ONE adapter trained on both tasks at once, from the pretrained base
run_job train_baseline_joint train "$(wc -l < $EXP/data/train_joint.jsonl)" $NGPU <<CMD
$(train_cmd "$BASE_MODEL" "$EXP/data/train_joint.jsonl" "$EXP/adapters/baselines/joint_8b_seed${SEED}" "$NEW_VAL")
CMD
